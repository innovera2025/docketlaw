...
withDefaults(
  defineProps<{
    ...
    actionText?: string
    variant?: 'danger' | 'primary' | 'success' | 'warning' | 'blue'
    size?: 'sm' | 'md' | 'lg' | 'smm' | 'xl' | 'xxl'
    disabled?: boolean
...
<template>
  <div
    v-if="open"
    class="fixed left-0 top-0 z-60 flex h-screen w-screen overflow-auto"
    :class="[
      {
  ...
  >
    <div
      ...
      :class="[
        { 'm-8  rounded-[10px] shadow-lg': position === 'center' },
    ...
    >
      ...
      >
        ...
        <div v-if="position !== 'center' && actionButton" class="flex gap-x-[15px] items-start">
          ...
          <button
            :class="[
              'h-[50px] px-[30px] rounded-[10px] text-[14px] font-semibold text-white disabled:cursor-not-allowed disabled:bg-grey-light',
              variant === 'danger'
      ...
      <div
        v-if="position === 'center'"
        class="flex justify-end gap-x-[15px]"
        :class="{ 'p-5': actionButton }"
      >
        <button
          class="h-[50px] w-[85px] rounded-[10px] text-[15px] font-semibold text-grey disabled:cursor-not-allowed disabled:bg-grey-light bg-[#EDEFF1]"
          @click="
            () => {
              Emits('secondary')
              Emits('close')
        ...
        <button
          v-if="actionButton"
          :class="[
            'h-[50px] px-[30px] rounded-[10px] text-[15px] font-semibold text-white disabled:cursor-not-allowed disabled:bg-grey-light',
            variant === 'danger'
              ? 'bg-danger hover:bg-danger-hover'
              : variant === 'success'
                ? 'bg-success hover:bg-success/80'
