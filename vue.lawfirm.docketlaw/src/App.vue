...
const isBackSite = computed(() => {
  ...
  const prefixPaths = ['/profile']
  return exactPaths.includes(route.path) || prefixPaths.some((p) => route.path.startsWith(p))
})
...
const handleVerify = async () => {
  await userStore.verify().then(() => {
    ...
    const can = (key: string) => !!Number(perms?.[key])

    const navItems: typeof appNavs.value = []

    if (can('is_sec_report')) {
      navItems.push({
    ...
    if (can('is_sec_calendar')) {
      navItems.push({ title: 'ปฏิทินนัดหมาย', icon: 'Calendar', path: '/calendar' })
    }

    if (can('is_sec_cases')) {
      const caseChildren = [{ path: '/cases', name: 'รายการคดี', isShow: true }]
      if (can('is_view_case_steps')) caseChildren.push({ path: '/case-steps', name: 'จัดการขั้นตอนของคดี', isShow: true })
      navItems.push({ title: 'แฟ้มคดี', icon: 'DocumentV2', children: caseChildren })
    }

    if (can('is_sec_ocr')) {
      navItems.push({ title: 'ระบบค้นหาและอ่านเอกสาร OCR', icon: 'Book', path: '/ocr' })
    }

    if (can('is_sec_documents')) {
      navItems.push({ title: 'ระบบจัดเก็บข้อมูล', icon: 'File', path: '/storage' })
    }

    if (can('is_sec_employee')) {
      navItems.push({
        category: navItems.some((n) => n.category) ? undefined : 'การจัดการข้อมูล',
        title: 'ข้อมูลพนักงาน, ทนาย',
        icon: 'Profile',
        path: '/employee',
      })
    }

    if (can('is_sec_package')) {
      navItems.push({ title: 'แพ็กเกจและการชำระเงิน', icon: 'Package', path: '/packages' })
    }

    appNavs.value = navItems
    ...
    const matchedPrefix = Object.keys(PERM_MAP).find(
      (prefix) => route.path === prefix || route.path.startsWith(prefix + '/'),
    )
    if (matchedPrefix && !can(PERM_MAP[matchedPrefix])) {
      router.replace('/')
    }
  })
}
...
watch([forceOpenNav, forceHideNav], () => {
  ...
  if (isSmallScreen && forceOpenNav.value && !forceHideNav.value) {
...
const handleResize = () => {
  const isSmallScreen = window.innerWidth < 1024
  if (isSmallScreen && forceHideNav.value) {
...
watch(route, async () => {
  if (route.path === '/login' || route.path === '/register' || route.path === '/forget-password')
    return
  await handleVerify()
...
<template>
  <div v-if="isEnv">
    <div
      v-if="
        route.path !== '/login' && route.path !== '/register' && route.path !== '/forget-password'
      "
            ...
            <div class="flex items-center gap-[15px]">
              ...
              >
                ...
                <span
                  ...
                  :class="[
                    {
                      'cursor-pointer !text-primary': route.meta.routes[i],
                    },
                  ]"
                >
                  <p v-if="i > 0">-</p>
